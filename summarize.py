...
parser.add_argument('--sort', action='store_true', default=False, help='sort')
...
for k, v in stats.items():
    ...
    txt = space('{} {:.3f}+{:.3f} (ep{}) {:.3f}+{:.3f} (ep{}) Mean {:.3f}'.format(k, v[min_epoch], v_interval[min_epoch], min_epoch, v[max_epoch], v_interval[max_epoch], max_epoch, np.mean(v)))
    if k.startswith(valid_string):
        ...
        if k_test in stats:
            ...
            txt += space('{} {:.3f}+{:.3f} (ep{}) {:.3f}+{:.3f} (ep{}) Mean {:.3f}'.format(k_test, v_test[min_epoch], v_test_interval[min_epoch], min_epoch, v_test[max_epoch], v_test_interval[max_epoch], max_epoch, np.mean(v_test)))
        else:
